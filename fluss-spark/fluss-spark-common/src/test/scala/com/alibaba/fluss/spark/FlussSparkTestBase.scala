...
import org.apache.spark.fluss.Utils
...
class FlussSparkTestBase
  ...
  protected def writeRowsToPartition(
      tablePath: TablePath,
      partitions: ju.Collection[String]): ju.List[String] = {
    val rows: ju.List[InternalRow] = new ju.ArrayList[InternalRow]
    val expectedRowValues: ju.List[String] = new ju.ArrayList[String]
    for (partition <- partitions.asScala) {
      for (i <- 0 until 10) {
        ...
        expectedRowValues.add(s"+I[$i, v1,$partition]")
      }
    }
    // write records
