...
import com.alibaba.fluss.spark.{FetchedData, TableBucketInfo}
 ...
 * class cannot be replaced with general pool implementations including Apache Commons Pool which
 * pools KafkaConsumer.
 */
class FetchedDataPool(executorService: ScheduledExecutorService, clock: Clock, conf: SparkConf)
  ...
  private val minEvictableIdleTimeMillis = 300
  private val evictorThreadRunIntervalMillis = 60
...
object FetchedDataPool {
  ...
  private case class CachedFetchedData(fetchedData: FetchedData) {
