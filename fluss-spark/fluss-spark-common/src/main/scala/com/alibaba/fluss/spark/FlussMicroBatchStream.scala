...
import org.apache.spark.sql.fluss.{BucketOffsetMap, ClockUtils, FlussClock, MockedSystemClock}
...
private class FlussMicroBatchStream(
  ...
  private val maxOffsetsPerTrigger =
    Option(options.get(SparkConnectorOptions.MAX_OFFSET_PER_TRIGGER)).map(_.toLong)

  private val minOffsetPerTrigger =
    Option(options.get(SparkConnectorOptions.MIN_OFFSET_PER_TRIGGER)).map(_.toLong)
  val maxTriggerDelayMs =
    JavaUtils.timeStringAsMs(
      Option(options.get(SparkConnectorOptions.MAX_TRIGGER_DELAY)).getOrElse("15m"))
  ...
  private val clock: FlussClock = if (options.containsKey(SparkConnectorOptions.MOCK_SYSTEM_TIME)) {
    new MockedSystemClock()
  } else {
    ClockUtils.systemClock()
  }
  ...
  def getOffsetRangesFromResolvedOffsets(
    ...
    untilPartitionOffsets.foreach {
      case (tbInfo, offset) =>
        val tb = tbInfo.getTableBucket
        val fromSnapshotId = fromPartitionOffsets
          .find { case (ftbInfo, _) => ftbInfo.getTableBucket.equals(tb) }
          .map(_._1.getSnapshotId)
          .getOrElse(null)
        tbInfo.setSnapshotId(fromSnapshotId)
    }
...
//        newPartitionInitialOffsets.contains(tp) ||
    ...
    val ranges = bucketPartitions.map {
      bp =>
        val fromOffset = fromOffsets(bp)
  ...
  def fetchLatestOffsets(optionalPartitionOffsetMap: Option[BucketOffsetMap]): BucketOffsetMap = {
    var latestBucketOffsetMap: Map[TableBucketInfo, Long] = Map.empty
    if (optionalPartitionOffsetMap.nonEmpty) {
      val partitionOffsetMap = optionalPartitionOffsetMap.get
      val partitionNames = partitionOffsetMap.map(entry => entry._1.getPartitionName).toArray
      val buckets = partitionOffsetMap
        .map(entry => Integer.valueOf(entry._1.getTableBucket.getBucket))
        .toArray
        .toList
        .asJava
      val intToBucket =
        partitionOffsetMap.map(entry => entry._1.getTableBucket.getBucket -> entry._1)
      partitionNames.foreach(
        partitionName => {
          var tempMap: Map[TableBucketInfo, Long] = Map.empty
          val latestBucketOffset = stoppingOffsetsInitializer.getBucketOffsets(
            partitionName,
            buckets,
            bucketOffsetsRetriever)
          latestBucketOffset.asScala.foreach(
            entry => tempMap += (intToBucket(entry._1) -> entry._2))
          if (hasPrimaryKey) {
            try {
              ...
              tempMap.foreach {
                case (tbInfo, offset) => (
                  tbInfo.setSnapshotId(
                    kvSnapshots
                      .getSnapshotId(tbInfo.getTableBucket.getBucket)
                      .orElse(null.asInstanceOf[Long]))
                  )
              }
            } catch {
              case e: Exception =>
          ...
          latestBucketOffsetMap = latestBucketOffsetMap ++ tempMap
        })
    } else {
      latestBucketOffsetMap = initLatestLogTableSplits()
    }

    latestBucketOffsetMap
  ...
  private def initLatestLogTableSplits(): BucketOffsetMap = {
    var partitionOffsetMap: Map[TableBucketInfo, Long] = Map.empty
    if (isPartitioned) {
      val partitions = flussAdmin.listPartitionInfos(tablePath).get
  ...
  private def initPrimaryKeyTablePartitionSplits(
    ...
    for (partitionInfo <- newPartitions) {
      ...
      var kvSnapshots: KvSnapshots = null
      try kvSnapshots = flussAdmin.getLatestKvSnapshots(tablePath, partitionName).get
      catch {
        case e: Exception =>
          throw new SparkRuntimeException(
            String.format(
  ...
  private def initNonPartitionedSplits(): BucketOffsetMap = {
    if (hasPrimaryKey && startingOffsetsInitializer.isInstanceOf[SnapshotOffsetsInitializer]) {
      // get the table snapshot info
      var kvSnapshots: KvSnapshots = null
      try kvSnapshots = flussAdmin.getLatestKvSnapshots(tablePath).get
      catch {
        case e: Exception =>
          throw new SparkRuntimeException(
  ...
  private def getLatestLogSplit(
    ...
    if (hasPrimaryKey) {
      try {
        ...
        partitionName match {
          case Some(name) =>
            kvSnapshots = flussAdmin.getLatestKvSnapshots(tablePath, partitionName.get).get
          case None => kvSnapshots = flussAdmin.getLatestKvSnapshots(tablePath).get
        }
  ...
  private def verifyEndOffsetForTriggerAvailableNow(
      endPartitionOffsets: Map[TableBucketInfo, Long]): Unit = {
