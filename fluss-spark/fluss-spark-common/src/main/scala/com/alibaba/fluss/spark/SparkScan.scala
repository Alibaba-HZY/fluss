...
class SparkScan(
  ...
  override def toMicroBatchStream(checkpointLocation: String): MicroBatchStream = {
    val configuration = SparkConversions.toFlussClientConfig(options)
  ...
  override def estimateStatistics(): Statistics = {
    ...
    val sizeInBytes = readSchema.defaultSize * numRows

    new Statistics {
      override def sizeInBytes(): OptionalLong = sizeInBytes

      override def numRows(): OptionalLong = numRows
    }
  }
