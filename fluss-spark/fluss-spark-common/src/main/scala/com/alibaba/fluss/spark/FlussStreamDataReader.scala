...
private class FlussDataConsumer(
  ...
  /**
   * Poll messages from Fluss starting from `offset` and update `fetchedData`. `fetchedData` may be
   * empty if the Fluss consumer fetches some messages but all of them are not visible messages
